...
/// Daftar Pelatih (API publik `/public/pelatih`) — infinite scroll + search
/// server-side, mekanisme sama dengan daftar Atlet. Hanya kolom aman.
class PelatihListPage extends StatefulWidget {
  /// Bila diisi, difilter ke pelatih cabor tersebut (via riwayat pengalaman).
  final String? caborId;
  final String? caborName;
  const PelatihListPage({super.key, this.caborId, this.caborName});

  @override
  State<PelatihListPage> createState() => _PelatihListPageState();
}

class _PelatihListPageState extends State<PelatihListPage> {
  ...
  /// Data pelatih hanya untuk pengguna login — cek token dulu sebelum fetch.
  ...
  Future<void> _fetch({String query = ''}) async {
    ...
    final data = await _apiService.fetchPelatihPage(page: 1, query: query, caborId: widget.caborId);
    if (!mounted) return;
    setState(() {
      _items = data;
      _page = 1;
  ...
  Future<void> _loadMore() async {
    ...
    final next = _page + 1;
    final data = await _apiService.fetchPelatihPage(page: next, query: _query, caborId: widget.caborId);
    if (!mounted) return;
    setState(() {
      _items.addAll(data);
      _page = next;
  ...
  Widget _body() {
    if (!_loggedIn) {
      return const LoginRequiredView(
        ...
        message: 'Anda harus login untuk melihat data pelatih.',
      );
    }
    if (_loading) {
    ...
    if (_items.isEmpty) {
      return const Center(
        child: Padding(
          ...
          child: Text('Pelatih tidak ditemukan', style: TextStyle(color: AppColors.muted)),
        ),
      );
    }
  ...
  Widget _listFooter() {
    ...
    if (!_hasMore) {
      return const Padding(
        ...
        child: Center(
          child: Text('— semua pelatih telah dimuat —',
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(widget.caborName != null ? 'Pelatih ${widget.caborName}' : 'Pelatih',
                        style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: Colors.white),
                        maxLines: 1, overflow: TextOverflow.ellipsis),
                    Text(widget.caborName != null ? 'Pelatih cabang ini' : 'Pelatih terdaftar KONI Kab. Tangerang',
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => PersonDetailPage(
          id: id,
          roleLabel: 'Pelatih',
          fetcher: _apiService.fetchPelatihDetail,
        ),
      ),
    );
  }

  Widget _pelatihCard(Map<String, dynamic> p) {
    final nama = (p['nama'] ?? '-').toString();
    final foto = (p['foto_url'] ?? '').toString();
    final gender = (p['jenis_kelamin'] ?? '').toString();
    final lahir = (p['tempat_lahir'] ?? '').toString();
  ...
  Widget _avatarFallback(String nama) {
    return Container(
      ...
      child: Text(_initials(nama),
          style: const TextStyle(color: AppColors.navy, fontWeight: FontWeight.w800, fontSize: 14)),
    );
  }
}
